resource "aws_kms_key" "encryption" {
  ...
  policy                  = templatefile("${path.module}/templates/${var.kms_policy}.json.tpl", { account_id = data.aws_caller_identity.current.account_id, region = data.aws_region.current.region })
}
