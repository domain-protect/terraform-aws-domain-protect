data "archive_file" "lambda_zip" {
  ...
  type        = "zip"
  source_dir  = "${abspath(path.root)}/build/lambda_dist_pkg_takeover"
  output_path = "${abspath(path.root)}/build/takeover.zip"
}
...
resource "aws_lambda_function" "lambda" {
  ...
  filename         = "${abspath(path.root)}/build/takeover.zip"
