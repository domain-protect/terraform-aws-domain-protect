...
ELASTICBEANSTALK_AUTOGENERATED_ENDPOINT = re.compile(
    r"^(?P<name>.+)\.(?P<subdomain>.+)\.(?P<region>[a-z0-9-]+\.)elasticbeanstalk\.com\.?$",
)
...
def eb_susceptible(domain):
    ...
    if is_elasticbeanstalk_url(domain):
        if is_aws_elasticbeanstalk_autogenerated_url(domain):
...
def is_aws_elasticbeanstalk_autogenerated_url(url):
    return url and ELASTICBEANSTALK_AUTOGENERATED_ENDPOINT.match(url)
