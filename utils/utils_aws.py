...
ELASTICBEANSTALK_ENDPOINT = re.compile(r"^(?P<name>.+)\.(?P<region>[a-z0-9-]+\.)elasticbeanstalk\.com\.?$")
...
def eb_susceptible(domain):
    ...
    if is_elasticbeanstalk_url(domain):
...
def is_elasticbeanstalk_url(url):
    return url and ELASTICBEANSTALK_ENDPOINT.match(url)
